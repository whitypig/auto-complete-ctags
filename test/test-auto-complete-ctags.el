...
(ert-deftest test-ac-ctags-build-completion-table:cpp-tags ()
  (let* ((db nil)
    ...
    (should (> (length (cadar tbl)) 0))
    (loop for n in (cadar tbl)
          do (unless (stringp n) (return nil))
          finally return t)))
...
(ert-deftest test-ac-ctags-build-completion-table:cpp-and-java-tags ()
  (let* ((db nil)
    ...
    (let ((cpp-tbl (assoc "C++" tbl)) (java-tbl (assoc "Java" tbl)))
      ...
      (should (> (length (cadr cpp-tbl)) 1))
      (should (loop for e in (cadr cpp-tbl)
                    do (unless (stringp e) (return nil))
                    finally return t))
      ;; java-tbl => ("Java" (n1 n2...))
      (should (string= "Java" (car java-tbl)))
      (should (> (length (cadr java-tbl)) 1))
      (should (loop for e in (cadr java-tbl)
                    do (unless (stringp e) (return nil))
                    finally return t)))))
